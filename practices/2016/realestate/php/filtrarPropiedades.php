...
$actividad  = filter_var($actividad , FILTER_SANITIZE_STRING); /*saneando la datos*/
$tipoPropiedad  = filter_var($tipoPropiedad , FILTER_SANITIZE_STRING); /*saneando la datos*/
$pais  = filter_var($pais , FILTER_SANITIZE_STRING); /*saneando la datos*/

//echo 'actividad : '.$actividad.',tipo: '.$tipoPropiedad.',pais: '.$pais;
...
if($pais != 'todas'){
	$cadena_filtros = $cadena_filtros.' AND propiedades.pais = "'.$pais.'" ';
}
...
$consulta = 'SELECT  propiedades.id, propiedades.usuario,propiedades.actividad, propiedades.tipopropiedad,
			...
			propiedades.tipo_area , propiedades.area , propiedades.path_poligono ,propiedades.linkalbum
			 FROM propiedades , usuarios WHERE propiedades.usuario = usuarios.id '.$cadena_filtros.' 
